...
export default function StablecoinTable({ data }: StablecoinTableProps) {
  return (
    <div className="glass-card overflow-hidden">
      <div className="px-5 py-4 border-b border-[var(--sol-border)]">
        <h3 className="text-[15px] font-semibold text-[var(--sol-text)]">
          Stablecoin Overview
        </h3>
        <p className="text-[12px] text-[var(--sol-text-muted)] mt-0.5">
      ...
      <div className="overflow-x-auto">
        <table className="w-full data-table">
          <thead>
            <tr className="text-[11px] text-[var(--sol-text-muted)] uppercase tracking-wider">
              <th className="text-left px-5 py-3 font-medium">#</th>
              <th className="text-left px-5 py-3 font-medium">Stablecoin</th>
              <th className="text-right px-5 py-3 font-medium">Supply</th>
              <th className="text-right px-5 py-3 font-medium">7d</th>
              <th className="text-right px-5 py-3 font-medium">30d</th>
              <th className="text-right px-5 py-3 font-medium">24h Volume</th>
              <th className="text-right px-5 py-3 font-medium">24h Txns</th>
              <th className="text-right px-5 py-3 font-medium">Wallets</th>
            ...
            {data.stablecoins.map((coin, index) => {
              ...
              return (
                <tr key={coin.symbol} className="group">
                  <td className="px-5 py-3.5 text-[12px] text-[var(--sol-text-muted)]">
                    {index + 1}
                  </td>
                  <td className="px-5 py-3.5">
                    ...
                    >
                      ...
                      <div>
                        <div className="text-[13px] font-semibold text-[var(--sol-text)] group-hover:text-[#9945FF] transition-colors">
                          {coin.symbol}
                        </div>
                        <div className="text-[11px] text-[var(--sol-text-muted)]">
                          {issuer?.issuer || coin.name}
                        </div>
                      </div>
                    </Link>
                  </td>
                  <td className="px-5 py-3.5 text-right text-[13px] font-semibold text-[var(--sol-text)]">
                  ...
                  <td className="px-5 py-3.5 text-right text-[13px] text-[var(--sol-text-secondary)]">
